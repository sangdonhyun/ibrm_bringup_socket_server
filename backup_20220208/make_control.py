...
""".format(CLONE_DATA_DIR1=clone_data_dir1,CLONE_DATA_DIR2=clone_data_dir2,ORACLE_SID=self.oracle_sid)
        ...
        cmd="""su - {OWNER} -c "export ORACLE_SID={SID};sqlplus  \'/as sysdba\' < {QUERY_FILE}" """.format(OWNER=self.owner,SID=self.oracle_sid,QUERY_FILE=query_file)
        print('CMD',cmd)
...
catalog start with '/ZFS/CLONEDB1/ARCH_ORCL_12' noprompt;
        ...
        cmd="""su - {OWNER} -c "export ORACLE_SID={SID};rman target / nocatalog < {QUEYR_FILE}" """.format(OWNER=self.owner, SID=self.oracle_sid, QUEYR_FILE=catalog_sql)
        ret=os.popen(cmd).read()
        print(ret)
