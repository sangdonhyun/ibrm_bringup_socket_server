...
class bring_up():
    def __init__(self,snap_info):
        self.oracle_sid = snap_info['oraSid']
        self.oracle_home = snap_info['oraHome']
        self.owner = self.get_owner()
        self.oracle_ver = snap_info['oraVer'].split()[-1]
        self.evn = os.environ
        self.data_dir = 'DATA_ORCL_11'
        self.arch_dir = 'ARCH_ORCL_11'
        self.data_dir2 = 'DATA_ORCL_12'
        self.arch_dir2 = 'ARCH_ORCL_12'
...
*.compatible='{ORACLE_VER}'
*.control_files='/ZFS/{ORACLE_SID}/{DATA_DIR}/new_control01.ctl'
*.db_block_size=8192
*.db_domain=''
*.db_name='{ORACLE_SID}'
*.db_recovery_file_dest='/ZFS/{ORACLE_SID}/{ARCH_DIR}'
*.db_recovery_file_dest_size=1024m
*.open_cursors=300
*.pga_aggregate_target=500m
*.processes=150
...
*.undo_tablespace='UNDOTBS1'
*.db_files=2048
log_archive_dest_1='location=/ZFS/{ORACLE_SID}/{ARCH_DIR}'
log_archive_format='arch_%s_%t_%r.arc'
log_buffer=104857600
nls_date_format='YYYY/MM/DD HH24:MI:SS'
nls_language='AMERICAN'
nls_territory='AMERICA'
""".format(ORACLE_VER=self.oracle_ver,ORACLE_SID=self.oracle_sid,DATA_DIR=self.data_dir,ARCH_DIR=self.arch_dir)
...
DATAFILE
 ...
 '/ZFS/CLONEDB1/DATA_ORCL_12/DCH4/data_D-ORCL_I-1594115525_TS-USERS_FNO-7_sn0gsksd';
        """
        list_file = '/ZFS/{ORACLE_SID}/{DATA_DIR}/list_data.out'.format(ORACLE_SID=self.oracle_sid,DATA_DIR=self.data_dir)
        # list_file = 'list_data.out'

        with open(list_file) as f:
            line_set = f.readlines()
        clone_data_dir1 = '/ZFS/{ORACLE_SID}/{DATA_DIR}'.format(ORACLE_SID=self.oracle_sid,DATA_DIR=self.data_dir)
        clone_data_dir2 = '/ZFS/{ORACLE_SID}/{DATA_DIR2}'.format(ORACLE_SID=self.oracle_sid,DATA_DIR2=self.data_dir2)
        ctr_str = """
SHUTDOWN ABORT;
STARTUP NOMOUNT
