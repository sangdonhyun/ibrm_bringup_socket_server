...
echo "sed '1,\$s/BACKUP/${CLONEDIR}/g'  \${CLONE_DATA_DIR1}/list_data.out | \
      sed '1,\$s/${BACKUP_DATA_1}/${CLONE_DATA_1}/g'| \
...
echo "Copy init file from ${DBADIR}/init${ORACLE_SID}.ora to ${ORACLE_HOME}/dbs/ "  						>> ${CLONEDB_Recover_LOG}

cp -rp ${DBADIR}/init${ORACLE_SID}.ora ${ORACLE_HOME}/dbs/init${ORACLE_SID}.ora 						>> ${CLONEDB_Recover_LOG}

echo "********************************************************************************************************************"  	>> ${CLONEDB_Recover_LOG}
echo ""																>> ${CLONEDB_Recover_LOG}
echo "********************************************************************************************************************"  	>> ${CLONEDB_Recover_LOG}
echo "Create control file for CLONEDB "  											>> ${CLONEDB_Recover_LOG}
echo "--------------------------------------------------------------------------------------------------------------------"  	>> ${CLONEDB_Recover_LOG}
...
echo "**[ End of Step 1 ]*************************************************************************************************"  	>> ${CLONEDB_Recover_LOG}
