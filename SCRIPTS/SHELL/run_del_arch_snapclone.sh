...
echo -n "Enter your choice ( "Local" or "Replicate" ) : "
     read input
          ...
          if [ "$input" = L ]
              then
                echo    "Archive log Clone in the Project [ ${ARCH_PROJNAME} ] will be deleted."
                     ...
                     if [ "$yesno" = Y ]
                        then
                         i=1
                            while [ $i -le $ARCH_NO ]
                             do
                                ZFS1="\${ZFSDIR}/del_clone.sh \${LOGINSTRING_${i}} \${POOLNAME_${i}} \${ARCH_PROJNAME} \${SHARE_CLONE_ARCH_${i}} " ; echo $ZFS1 | sh
                                i=$(( $i + 1 ))
                             done
                   else
