...
echo -n "Please enter [y] to continue : "
  read yesno
	...
	do
          ZFS="\${ZFSDIR}/rename_snap.sh \${LOGINSTRING_${i}} \${POOLNAME_${i}} \${DATA_PROJNAME} \${SHARE_DATA_${i}} ${datasnap} CLONE_${datasnap} " ; echo $ZFS | sh
          ZFS1="\${ZFSDIR}/clone.sh \${LOGINSTRING_${i}} \${POOLNAME_${i}} \${DATA_PROJNAME} \${SHARE_DATA_${i}} CLONE_${datasnap} \${SHARE_CLONE_DATA_${i}} " ; echo $ZFS1 | sh
            i=$(( $i + 1 ))
	done
      else
